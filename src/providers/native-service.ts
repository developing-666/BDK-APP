...
export class NativeService {
	...
	showToast(options: any): void {
		...
		let opts: any = Utils.extend(true, {}, defaults, options);
		if (this.isMobile()) {
			this.toast
				.show(
					opts.message,
					String(opts.duration),
					opts.position === 'middle' ? 'center' : opts.position
				)
				.subscribe();
		} else {
	...
	sendSMS(num: string): any {
		return this.sms.send(num, '', this.isAndroid() ? {
			android: {
				intent: 'INTENT'
			}
		} : undefined);
