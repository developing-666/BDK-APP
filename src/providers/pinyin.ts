...
import pinyin_dict_notone from '../assets/lib/pinyin-dict';
...
export class Pinyin {
	...
	pinyin_dict_firstletter: any = null;
	pinyin_dict_notone: any = pinyin_dict_notone;
	pinyin_dict_withtone: any = null;
	pinyin_dict_polyphone: any = null;
	dict: any = {};
	constructor() {
		this.parseDict();
	}
	parseDict() {
		...
		if (this.pinyin_dict_firstletter) {
			this.dict.firstletter = this.pinyin_dict_firstletter;
		}
		...
		if (this.pinyin_dict_withtone) {
			...
			var temp = this.pinyin_dict_withtone.split(',');
			...
			if (this.pinyin_dict_notone) {
				...
				var notone = this.removeTone(this.pinyin_dict_withtone).split(',');
				var py2hz = {}, py, hz;
	 ...
	 */
	getPinyin(chinese, splitter:string = ' ', withtone:boolean = false, polyphone:boolean = false) {
	 ...
	 */
	removeTone(pinyin) {
		return pinyin.replace(/[āáǎàōóǒòēéěèīíǐìūúǔùüǖǘǚǜńň]/g, function(m) { return this.toneMap[m][0]; });
	}
	/**
	 * 将数组拼音转换成真正的带标点的拼音
	 * @param pinyinWithoutTone 类似 xu2e这样的带数字的拼音
	 */
	getTone(pinyinWithoutTone: any = '') {
		let newToneMap = {};
		for (let i in this.toneMap) {
			newToneMap[this.toneMap[i]] = i;
		};
	...
	parsePolyphone(chinese, result, splitter, withtone) {
		var poly = this.pinyin_dict_polyphone;
