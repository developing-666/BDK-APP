...
export class IonInputPanelComponent implements OnInit {
	...
	delete(e) {
		if (this.platform.is('ios')) {
			this.file.removeFile(this.file.cacheDirectory, this.fileName).then(()=>{
				this.complete = false;
	...
	startRecording() {
		...
		this.file.createFile(this.file.cacheDirectory, this.fileName, true).then(() => {
			this.mediaObj = this.media.create(this.file.cacheDirectory.replace(/^file:\/\//, '') + this.fileName);
	...
	recordSuccess(mediaObj: MediaObject) {
		if (this.seconds<=2){
			...
			this.complete = true;
			this.applicationRef.tick();
			this.doneRecord(this.file.cacheDirectory.replace(/^file:\/\//, '') + this.fileName);
		}
	}
	doneRecord(src) {
